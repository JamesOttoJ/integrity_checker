...
int main(int argc, char* argv[]) {
  ...
  int bytes_read, bytes_written, bytes_encrypted;
  FILE* input_file;
  FILE* output_file;
  int input_file_size;
  char* file_name;
...
//   unsigned char ivec[] = {0xfc, 0x9d, 0x17, 0x0f, 0x36, 0xfa, 0x90, 0x03, 0x75, 0x94, 0x71, 0x81, 0xd9, 0x18, 0x3d, 0x52, 0x16, 0xe8, 0x9a, 0x72, 0xdd, 0x7b, 0x41, 0x8a, 0x3a, 0x4c, 0x71, 0xb3, 0x0e, 0xd5, 0xf3, 0x6a};
  ...
  bytes_encrypted = encrypt(indata, input_file_size, ckey, ivec, outdata);
  
  fclose(input_file);
  output_file = fopen(file_name, "wb");

  bytes_written = fwrite(outdata, 1, bytes_encrypted, output_file);
  if (bytes_written < 1) {
